...
> 状态：`IMPLEMENTED / V3 ACCEPTED`（历史设计基线）
>
...
> 后续方向：继续沿“单模板做深”推进公开试玩、匿名遥测、建议到候选版本和 A/B 对比；RAG 升级为真实、持久、可评测的语义检索。当前工作包见 [B 路线与 RAG 升级路线图](roadmap-balance-lab-rag.md)。本文保留 V3 设计边界，不作为下一阶段需求总纲。

## 1. 版本定位

V2 解决了 AI 工作流的工程化问题：Java 异步编排、RabbitMQ、Outbox、幂等、恢复、SSE、RAG、评测和可观测性。V2.1 负责补齐普通用户可使用的前端入口。

V3 不建设通用游戏引擎，而是把现有“AI 生成参数后套入固定 Phaser 模板”的 Demo，升级为受限玩法的轻量原型与实验工具：

> 将一句受限玩法创意转化为一个 3-10 分钟可试玩、可调参、可评测、可复现、可导出的 H5 原型垂直切片。
