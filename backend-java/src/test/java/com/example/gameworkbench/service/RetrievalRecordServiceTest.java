...
class RetrievalRecordServiceTest {
    @Test void deniesCrossProjectRun() { var records = mock(RetrievalRecordMapper.class); var runs = mock(AgentRunMapper.class); assertThat(new RetrievalRecordService(records, runs).list(1L, 2L, 3L)).isEmpty(); verify(records, never()).selectList(any()); }
    @Test void persistsOnlyActualUsedReferencesAndHashesOnlyTheQuery() throws Exception {
        ...
        RetrievalRecordService service = new RetrievalRecordService(records, mock(AgentRunMapper.class));
    ...
    @Test void ragOffWritesNoFakeReferences() { var records = mock(RetrievalRecordMapper.class); AgentRun run = new AgentRun(); run.setRagEnabled(false); new RetrievalRecordService(records, mock(AgentRunMapper.class)).recordSelected(run, null, "x"); verifyNoInteractions(records); }
}
