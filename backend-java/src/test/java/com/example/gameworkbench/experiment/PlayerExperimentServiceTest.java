...
class PlayerExperimentServiceTest {
    @Test void deterministicComparisonRecommendsOnlyWhenTargetAndGuardrailPass(){Fixture f=fixture();add(f,"l1",episode("l1","base","NOVICE",1,"WON",100));add(f,"l2",episode("l2","base","EXPERT",2,"WON",100));add(f,"l3",episode("l3","base","NOVICE",3,"ERROR",100));add(f,"r1",episode("r1","candidate","NOVICE",1,"WON",105));add(f,"r2",episode("r2","candidate","EXPERT",2,"WON",105));add(f,"r3",episode("r3","candidate","NOVICE",3,"WON",105));var args=f.json.createObjectNode().put("baselineVersionUuid","base").put("candidateVersionUuid","candidate").put("minimumSamples",3).put("minimumCompletionRate",.8).put("maxExpertMeanTimeDelta",.08);args.putArray("baselineEpisodeUuids").add("l1").add("l2").add("l3");args.putArray("candidateEpisodeUuids").add("r1").add("r2").add("r3");var result=f.service.compare(7,1,"run",args);assertThat(result.path("comparable").asBoolean()).isTrue();assertThat(result.path("recommended").asBoolean()).isTrue();ArgumentCaptor<ExperimentComparison> saved=ArgumentCaptor.forClass(ExperimentComparison.class);verify(f.comparisons).insert(saved.capture());assertThat(saved.getValue().getEpisodeRefsJson()).contains("l1","r3");}
    @Test void partialEpisodeMakesWindowExplicitlyNotComparable(){Fixture f=fixture();MachineEpisode partial=episode("l1","base","NOVICE",1,"WON",100);partial.setExecutionStatus("FAILED");add(f,"l1",partial);add(f,"r1",episode("r1","candidate","NOVICE",1,"WON",100));var args=f.json.createObjectNode().put("baselineVersionUuid","base").put("candidateVersionUuid","candidate").put("minimumSamples",1).put("minimumCompletionRate",0).put("maxExpertMeanTimeDelta",1);args.putArray("baselineEpisodeUuids").add("l1");args.putArray("candidateEpisodeUuids").add("r1");assertThat(f.service.compare(7,1,"run",args).path("reason").asText()).isEqualTo("PARTIAL_OR_FAILED_EPISODES");}
