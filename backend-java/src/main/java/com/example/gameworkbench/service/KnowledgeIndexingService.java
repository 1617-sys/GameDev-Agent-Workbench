...
public class KnowledgeIndexingService {
 ...
 public void index(KnowledgeDocument document, String extractedText) {
  ...
  if (chunks.selectCount(new LambdaQueryWrapper<KnowledgeChunk>().eq(KnowledgeChunk::getProjectId,document.getProjectId()).eq(KnowledgeChunk::getDocumentId,document.getId()).eq(KnowledgeChunk::getChunkingVersion,KnowledgeChunker.VERSION).eq(KnowledgeChunk::getEmbeddingModel,embeddingProvider.model()))>0) return;
 ...
 public void invalidate(KnowledgeDocument document){ for(KnowledgeChunk c:chunks.selectList(new LambdaQueryWrapper<KnowledgeChunk>().eq(KnowledgeChunk::getProjectId,document.getProjectId()).eq(KnowledgeChunk::getDocumentId,document.getId()))){ if(c.getVectorRef()!=null) vectorStore.delete(c.getVectorRef()); c.setIndexStatus("INVALID"); chunks.updateById(c); } }
