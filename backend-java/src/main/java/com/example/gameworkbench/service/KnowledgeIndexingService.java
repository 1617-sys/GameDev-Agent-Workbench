...
public class KnowledgeIndexingService {
 private final KnowledgeChunker chunker; private final EmbeddingProvider embeddingProvider; private final VectorStore vectorStore; private final KnowledgeChunkMapper chunks;
 public void index(KnowledgeDocument document, String extractedText) {
  ...
  for(int i=0;i<texts.size();i++){ String uuid=UUID.randomUUID().toString(); String ref="knowledge/"+document.getProjectId()+"/"+uuid; Map<String,String> meta=Map.of("projectId",String.valueOf(document.getProjectId()),"documentUuid",document.getDocumentUuid(),"documentVersion",String.valueOf(document.getVersion()),"chunkUuid",uuid,"status","ACTIVE","embeddingModel",embeddingProvider.model(),"dimension",String.valueOf(embeddingProvider.dimension())); vectorStore.upsert(ref,vectors.get(i),meta); KnowledgeChunk c=new KnowledgeChunk(); c.setChunkUuid(uuid); c.setDocumentId(document.getId()); c.setProjectId(document.getProjectId()); c.setOrdinal(i); c.setTextHash(hash(texts.get(i))); c.setTokenCount((texts.get(i).length()+3)/4); c.setChunkingVersion(KnowledgeChunker.VERSION); c.setEmbeddingModel(embeddingProvider.model()); c.setEmbeddingDimension(embeddingProvider.dimension()); c.setIndexStatus("INDEXED"); c.setVectorRef(ref); chunks.insert(c); }
 }
