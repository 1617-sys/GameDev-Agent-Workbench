...
/** Establishes a safe trace identifier without logging request headers or bodies. */
@Component
...
public class CorrelationIdFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        String candidate = request.getHeader(TRACE_HEADER);
        String traceId = candidate != null && SAFE_TRACE_ID.matcher(candidate).matches()
                ? candidate : UUID.randomUUID().toString();
        response.setHeader(TRACE_HEADER, traceId);
        try (DiagnosticContext ignored = DiagnosticContext.trace(traceId)) {
