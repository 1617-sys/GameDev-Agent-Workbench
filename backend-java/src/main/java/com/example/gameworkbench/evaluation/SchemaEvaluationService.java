...
public class SchemaEvaluationService {
 private final SchemaEvaluator evaluator; private final GameConfigRuleEvaluator ruleEvaluator; private final EvaluationReportMapper mapper; private final ObjectMapper objectMapper;
 public void evaluateAndPersist(AgentArtifact artifact) {
  ...
  if (result.passed()) persistRule(artifact);
 }
 private void persistRule(AgentArtifact artifact){ RuleEvaluationResult result=ruleEvaluator.evaluate(artifact.getContent()); EvaluationReport report=new EvaluationReport(); report.setArtifactId(artifact.getId()); report.setEvaluatorType("RULE"); report.setStatus(result.status()); report.setSchemaKey(artifact.getSchemaKey()); report.setSchemaVersion(artifact.getSchemaVersion()); report.setRuleVersion(result.ruleVersion()); report.setInputHash(sha256(artifact.getContent())); report.setEvaluationAttempt(1); report.setEvaluatedAt(LocalDateTime.now()); try{report.setViolationsJson(objectMapper.writeValueAsString(result.violations()));}catch(Exception e){report.setViolationsJson("[]");report.setStatus("ERROR");} mapper.insert(report); }
