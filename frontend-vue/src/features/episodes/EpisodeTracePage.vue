    ...
    <template v-else>
      ...
      <div class="evidence-layout">
        ...
        <main class="trace-panel">
          <div v-if="selectedRun && !selectedRun.persistedBatchUuid" class="empty-panel"><h2>{{ selectedRun.status }}</h2><p v-if="selectedRun.errorCode">{{ selectedRun.errorCode }}：{{ selectedRun.errorMessage }}</p><p v-else>运行尚未生成可持久化 Episode。</p></div>
          <template v-else-if="batch">
            ...
            <template v-if="episode">
              ...
              <nav v-if="stepPage?.total > stepPage?.size" class="pagination"><button :disabled="stepPage.page===0" @click="loadSteps(stepPage.page-1)">上一页</button><span>{{ stepPage.page+1 }} / {{ Math.ceil(stepPage.total/stepPage.size) }}</span><button :disabled="(stepPage.page+1)*stepPage.size>=stepPage.total" @click="loadSteps(stepPage.page+1)">下一页</button></nav>
            </template>
          </template>
        </main>
      </div>
...
async function selectRun(run){selectedRun.value=run;batch.value=null;episode.value=null;stepPage.value=null;if(!run.persistedBatchUuid)return;try{batch.value=await episodesApi.batch(projectUuid.value,run.persistedBatchUuid);if(batch.value.items?.length)await selectEpisode(batch.value.items[0].episodeId);}catch(cause){error.value=message(cause,"无法读取批次，可能没有权限");}}
async function selectEpisode(id){try{episode.value=await episodesApi.summary(projectUuid.value,id);await loadSteps(0);}catch(cause){error.value=message(cause,"无法读取 Episode 摘要");}}
