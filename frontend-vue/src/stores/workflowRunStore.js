...
export function createWorkflowRunStore({ api, eventSourceFactory = (url) => new EventSource(url) }) {
  ...
  const ensure = (uuid) => state.runs[uuid] ||= { snapshot: null, steps: [], artifacts: [], lastSequence: 0, connectionState: "idle", loading: false, error: null, source: null };
  ...
  return { state, ensure, loadRun, applySnapshot, applyEvent, connect, disconnect };
}
