"""Deterministic, network-isolated Agent fixture used only by the R7 E2E Compose override."""
...
VALID_GAME_CONFIG = {
    "version": "1.0",
    "title": "R7 fixture: Crystal Relay",
    ...
    "enemies": [{"id": "patrol-1", "x": 420, "y": 220, "speed": 90, "patrolAxis": "x", "patrolDistance": 180}],
...
def fixture_output(path: str, payload: dict) -> dict:
    if path.endswith("/game-config-generate"):
        if "E2E_INVALID_CONFIG" in (payload.get("content") or ""):
            return {
                "version": "1.0",
                "title": "R7 invalid fixture",
                "gameType": "unsupported_fixture_type",
    ...
    return {
        "fixture": "r7-e2e-fixed-agent-v1",
        "agent_path": path.rsplit("/", 1)[-1],
        "summary": "Deterministic mock output for the R7 release-hardening harness.",
    }


class Handler(BaseHTTPRequestHandler):
    server_version = "R7E2EFixture/1.0"

    def log_message(self, _format: str, *_args: object) -> None:
        # Do not log request bodies: they can contain test prompts or fixture data.
        return
    ...
    def do_POST(self) -> None:
        ...
        trace_seed = f"{payload.get('project_uuid', '')}|{payload.get('content', '')}|{self.path}".encode("utf-8")
        ...
        self._json(
            HTTPStatus.OK,
            {
                ...
                "data": {
                    "status": "SUCCESS",
                    "output": fixture_output(self.path, payload),
                    "raw_output_ref": None,
                    "model": "r7-e2e-fixed-agent-v1",
                    "provider": "fixture",
                    "usage": None,
                    "latency_ms": 5,
                    "mock": True,
                    "rag_status": "DISABLED" if not (payload.get("rag") or {}).get("rag_enabled") else ("AVAILABLE" if references else "EMPTY"),
