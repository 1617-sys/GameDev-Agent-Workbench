...
-- 1. 用户表
create table gamedev_agent_workbench.sys_user
(
...
-- 2. 游戏项目表
create table gamedev_agent_workbench.game_project
(
    ...
    constraint fk_game_project_user_id
        foreign key (user_id) references gamedev_agent_workbench.sys_user (id)
)
    comment '游戏项目表';

create index idx_game_project_status
    on gamedev_agent_workbench.game_project (status);

create index idx_game_project_user_id
    on gamedev_agent_workbench.game_project (user_id);

-- 3. Agent 执行记录表
create table gamedev_agent_workbench.agent_run
(
...
create index idx_agent_run_agent_type
    on gamedev_agent_workbench.agent_run (agent_type);

create index idx_agent_run_project_id
    on gamedev_agent_workbench.agent_run (project_id);

create index idx_agent_run_project_uuid
    on gamedev_agent_workbench.agent_run (project_uuid);

create index idx_agent_run_status
    on gamedev_agent_workbench.agent_run (status);

create index idx_agent_run_user_created_at
    on gamedev_agent_workbench.agent_run (user_id, created_at);

-- 4. Agent 产物表
create table gamedev_agent_workbench.agent_artifact
(
...
create index idx_agent_artifact_agent_run_id
    on gamedev_agent_workbench.agent_artifact (agent_run_id);

create index idx_agent_artifact_artifact_type
    on gamedev_agent_workbench.agent_artifact (artifact_type);

create index idx_agent_artifact_project_id
    on gamedev_agent_workbench.agent_artifact (project_id);

-- 5. Workflow 执行记录表
create table gamedev_agent_workbench.workflow_run
(
    id                bigint auto_increment comment '主键ID'
        primary key,
...
create index idx_workflow_run_project_id
    on gamedev_agent_workbench.workflow_run (project_id);

create index idx_workflow_run_status
    on gamedev_agent_workbench.workflow_run (status);

create index idx_workflow_run_user_id
    on gamedev_agent_workbench.workflow_run (user_id);

create index idx_workflow_run_workflow_type
    on gamedev_agent_workbench.workflow_run (workflow_type);

-- 6. Prompt 模板表
create table gamedev_agent_workbench.prompt_template
(
...
create index idx_prompt_template_agent_status
    on gamedev_agent_workbench.prompt_template (agent_type, status);

create index idx_prompt_template_agent_type
    on gamedev_agent_workbench.prompt_template (agent_type);

create index idx_prompt_template_status
    on gamedev_agent_workbench.prompt_template (status);
